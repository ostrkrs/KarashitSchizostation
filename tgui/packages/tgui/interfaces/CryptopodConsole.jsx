...
export const CryptopodConsole = (props) => {
  const { data } = useBackend();
  const { account_name } = data;

  const welcomeTitle = `Welcome ${account_name || 'Unknown'}!`;

  return (
    <Window title="Cryptosleep Oversight Console" width={400} height={480}>
      <Window.Content>
        <Stack vertical fill>
          <Stack.Item>
            <Section title={welcomeTitle}>
              This automated cryptosleep system will preserve your body until
              next destination.
            </Section>
          </Stack.Item>
          <Stack.Item grow>
...
const CrewList = (props) => {
  ...
  return (
    ...
    )) || <NoticeBox>No crew stored!</NoticeBox>
  );
};
...
const ItemList = (props) => {
  ...
  if (!item_retrieval_allowed) {
    return (
      <NoticeBox>You are not authorized to manage stored items.</NoticeBox>
    );
  }
  return (
    ...
    )) || <NoticeBox>No items stored!</NoticeBox>
  );
};
