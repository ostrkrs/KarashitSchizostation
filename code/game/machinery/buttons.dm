...
/obj/machinery/button/screwdriver_act(mob/living/user, obj/item/tool)
	...
	var/mutable_appearance/error = mutable_appearance(icon, "[base_icon_state]-overlay-error")
	error.dir = dir
	flick_overlay_view(error, 1 SECONDS)
	return ITEM_INTERACT_BLOCKING
...
/obj/machinery/button/proc/attempt_press(mob/user)
	...
	use_energy(5 JOULES)
	var/mutable_appearance/success = mutable_appearance(icon, "[base_icon_state]-overlay-success")
	success.dir = dir
	flick_overlay_view(success, 1 SECONDS)
