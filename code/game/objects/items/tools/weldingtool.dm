...
/obj/item/weldingtool
	...
	abstract_type = /obj/item/weldingtool
...
/obj/item/weldingtool/electric/use(used = 0)
	if(!isOn() || !check_energy())
		return FALSE
	var/amt = used ? used : power_use_amount
	if(inserted_cell.charge >= amt)
		inserted_cell.use(amt)
		check_energy()
		return TRUE
	else
		return FALSE

/obj/item/weldingtool/electric/use_tool(atom/target, mob/living/user, delay, amount=0, volume, datum/callback/extra_checks)
	var/amt = amount
	if(welding && amt == 0)
		amt = power_use_amount
	return ..(target, user, delay, amt, volume, extra_checks)
...
/obj/item/weldingtool/electric/super_cell
	inserted_cell = /obj/item/stock_parts/power_store/cell/super

/obj/item/weldingtool/electric/hyper_cell
	inserted_cell = /obj/item/stock_parts/power_store/cell/hyper

/obj/item/weldingtool/electric/bluespace_cell
	inserted_cell = /obj/item/stock_parts/power_store/cell/bluespace
