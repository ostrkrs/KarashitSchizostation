...
/obj/item/lighter/attack_self(mob/living/user)
	...
	if(fancy)
		...
		user.visible_message(
			span_warning("[user] tries to light [src], but the flame doesn't come out."),
			span_warning("You try to light [src], but the flame doesn't come out.")
		)
		playsound(src.loc , 'sound/items/lighter/lighter_off.ogg', 100, 1)
		return
...
/obj/item/lighter/slime/create_lighter_overlay()
	var/mutable_appearance/lighter_overlay = ..()
	lighter_overlay.color = COLOR_DARK_MODERATE_LIME_GREEN
	return lighter_overlay
...
/obj/item/lighter/bright/create_lighter_overlay()
	var/mutable_appearance/lighter_overlay = ..()
	lighter_overlay.color = LIGHT_COLOR_BABY_BLUE
	return lighter_overlay
