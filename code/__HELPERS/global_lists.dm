...
/proc/init_blooper_prefs()
	for(var/sound_blooper_path in subtypesof(/datum/blooper))
		...
		if(B.category == BLOOPER_CATEGORY_HUMAN)
			GLOB.blooper_list_humans[B.id] = sound_blooper_path

		if(B.category == BLOOPER_CATEGORY_SILICON)
			GLOB.blooper_list_silicons[B.id] = sound_blooper_path

		if(B.category == BLOOPER_CATEGORY_INSECT)
			GLOB.blooper_list_insects[B.id] = sound_blooper_path

		if(B.category == BLOOPER_CATEGORY_ANIMAL)
			GLOB.blooper_list_animals[B.id] = sound_blooper_path

		if(B.category == BLOOPER_CATEGORY_SKELETON)
			GLOB.blooper_list_skeletons[B.id] = sound_blooper_path
