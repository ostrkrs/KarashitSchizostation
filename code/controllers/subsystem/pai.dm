...
/datum/controller/subsystem/pai/ui_act(action, list/params, datum/tgui/ui)
	...
	switch(action)
		if("submit")
			candidate.comments = reject_bad_name(params["comments"], max_length = MAX_BROADCAST_LEN, strict = TRUE, cap_after_symbols = FALSE) || "Unknown"
			candidate.description = reject_bad_name(params["description"], max_length = MAX_BROADCAST_LEN, strict = TRUE, cap_after_symbols = FALSE) || "Unknown"
			candidate.name = reject_bad_name(params["name"], max_length = MAX_NAME_LEN, strict = TRUE, cap_after_symbols = FALSE) || "Unknown"
