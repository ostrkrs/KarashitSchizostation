...
/datum/unit_test/screenshot_digi/Run()
	...
	// doubles as coverage that cavers spawn with digitigrade legs (as they should be forced to do)
	dummy.set_species(/datum/species/human/cave_adapted)
	TEST_ASSERT((dummy.bodyshape & BODYSHAPE_DIGITIGRADE), "Dummy (Caver) should be digitigrade!")
	...
	TEST_ASSERT_EQUAL(dummy.wear_suit, suit, "Dummy (Caver) should be wearing the EVA suit!")
	finished_icon = icon(finished_icon)
	...
	TEST_ASSERT((suit in dummy.held_items), "Dummy (Caver) should be holding the EVA suit!")
	finished_icon = icon(finished_icon)
	finished_icon.Insert(getFlatIcon(dummy, no_anim = TRUE), dir = SOUTH, frame = 4)

	// screenshot of turning the caver into a human
	...
	// screenshot test of turning the human back into an caver
	// this should correctly update the auto gen sprites and leg sprites again
	dummy.set_species(/datum/species/human/cave_adapted)
	TEST_ASSERT((dummy.bodyshape & BODYSHAPE_DIGITIGRADE), "Dummy (Caver) should be digitigrade again!")
	finished_icon = icon(finished_icon)
	...
	dummy.equip_to_appropriate_slot(suit)
	TEST_ASSERT_EQUAL(dummy.wear_suit, suit, "Dummy (Caver) should be wearing the EVA suit again!")
	finished_icon = icon(finished_icon)
	...
	// should show the autogen'd legs once more
	qdel(suit)
	TEST_ASSERT_NULL(dummy.wear_suit, "Dummy (Caver) should not be wearing the EVA suit!")
	finished_icon = icon(finished_icon)
	...
	TEST_ASSERT_EQUAL(length(dummy.get_equipped_items()), 0, "Dummy (Caver) should have no equipment!")
	finished_icon = icon(finished_icon)
