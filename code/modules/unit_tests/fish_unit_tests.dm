...
/datum/unit_test/fish_portal_gen_linking/Run()
	...
	var/obj/machinery/fishing_portal_generator/unit_test/fishing_spot = new(get_turf(user)) //This is deleted during the test
	var/obj/structure/aquarium/extra_spot = allocate(/obj/structure/aquarium)
	...
	var/datum/fish_source/aquarium/fish_source = GLOB.preset_fish_sources[/datum/fish_source/aquarium]
...
/obj/machinery/fishing_portal_generator/unit_test/Initialize(mapload)
	. = ..()
	if(!HAS_TRAIT(src, TRAIT_FISHING_SPOT)) //Ensure this toilet has a fishing spot because only maploaded ones have it.
		AddComponent(/datum/component/fishing_spot, GLOB.preset_fish_sources[/datum/fish_source/aquarium])
