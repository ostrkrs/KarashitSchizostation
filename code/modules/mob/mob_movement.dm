...
///Verb to toggle the creep, walk or run status
/client/verb/toggle_walk_creep()
	set name = "toggle-walk-creep"
	set hidden = TRUE
	set instant = TRUE
	if(isliving(mob))
		var/mob/living/user_mob = mob
		user_mob.toggle_move_intent_creep_walk()
...
/client/verb/toggle_walk_run()
	...
	if(isliving(mob))
		var/mob/living/user_mob = mob
		user_mob.toggle_move_intent_walk_run()

///Toggle the move intent of the mob, triggers an update the move intent hud as well
/mob/living/proc/toggle_move_intent_creep_walk()
	if(move_intent != MOVE_INTENT_CREEP)
		move_intent = MOVE_INTENT_CREEP
	else
	...
	SEND_SIGNAL(src, COMSIG_MOVE_INTENT_CREEPWALK_TOGGLED)

/mob/living/proc/toggle_move_intent_walk_run()
	...
	SEND_SIGNAL(src, COMSIG_MOVE_INTENT_WALKRUN_TOGGLED)

///Cycles move intent
/mob/living/proc/cycle_move_intent()
	switch(move_intent)
