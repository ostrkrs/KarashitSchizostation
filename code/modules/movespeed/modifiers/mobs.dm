...
/datum/movespeed_modifier/config_creep_walk_run
	multiplicative_slowdown = 1
	id = MOVESPEED_ID_MOB_CREEP_WALK_RUN
	flags = IGNORE_NOSLOW

/datum/movespeed_modifier/config_creep_walk_run/proc/sync()

/datum/movespeed_modifier/config_creep_walk_run/creep/sync()
	var/mod = CONFIG_GET(number/movedelay/creep_delay)
	multiplicative_slowdown = isnum(mod)? mod : initial(multiplicative_slowdown)

/datum/movespeed_modifier/config_creep_walk_run/walk/sync()
	var/mod = CONFIG_GET(number/movedelay/walk_delay)
	multiplicative_slowdown = isnum(mod)? mod : initial(multiplicative_slowdown)

/datum/movespeed_modifier/config_creep_walk_run/run/sync()
