	...
	switch(action)
		if("power")
			...
			if(on)
				SSair.start_processing_machine(src)
			if(on && !holding)
				var/phoron = air_contents.gases[/datum/gas/phoron]
				var/n2o = air_contents.gases[/datum/gas/nitrous_oxide]
				if(n2o || phoron)
					message_admins("[ADMIN_LOOKUPFLW(usr)] turned on a pump that contains [n2o ? "N2O" : ""][n2o && phoron ? " & " : ""][phoron ? "Phoron" : ""] at [ADMIN_VERBOSEJMP(src)]")
					log_admin("[key_name(usr)] turned on a pump that contains [n2o ? "N2O" : ""][n2o && phoron ? " & " : ""][phoron ? "Phoron" : ""] at [AREACOORD(src)]")
