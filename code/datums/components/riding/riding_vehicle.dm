...
/datum/component/riding/vehicle/scooter/skateboard/vehicle_mob_buckle(datum/source, mob/living/rider, force = FALSE)
	...
	if(!can_slow_down)
		return
	RegisterSignal(rider, COMSIG_MOVE_INTENT_CREEPWALK_TOGGLED, PROC_REF(toggle_move_delay))
	if(rider.move_intent == MOVE_INTENT_CREEP)
...
/datum/component/riding/vehicle/scooter/skateboard/handle_unbuckle(mob/living/rider)
	...
	if(!can_slow_down)
		return
	UnregisterSignal(rider, COMSIG_MOVE_INTENT_CREEPWALK_TOGGLED)
	if(rider.move_intent == MOVE_INTENT_CREEP)
		vehicle_move_delay -= 0.6
